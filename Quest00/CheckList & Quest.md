...
##### 방법 1. `$ git rm -cached 파일명`
: 로컬파일은 그대로 두고 원격 저장소에 잘못 올라간 **파일만** 삭제
##### 방법 2. `$ git reset --hard 커밋해시`
: 돌아갈 **시점 이후**의 커밋이 삭제 ▸ 삭제 후 푸시하면 에러발생(원격저장소보다 로컬저장소가 뒤쳐지기때문) ▸ `$ git push -f origin 브랜치명` : **강제 푸시** ▸ 충돌발생위험높음
##### 방법 3. `$ git revert 커밋해시`
: 돌아가고 싶은 시점에 커밋 추가하는 방식으로 코드 되돌리기 ▸ 커밋 해당 내용만 삭제 ▸ 바로 커밋되게 하지 않으려면 `$ git revert --no-commit 커밋해시`
