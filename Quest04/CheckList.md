...
> 
> \- **정보은닉**을 하는 이유
> 1. 객체의 상태를 안정적으로 관리(예상하지 못한 변화를 예방)
> 2. 객체 간의 결합도를 낮춤
> 
...
> - 메서드와 데이터의 의미에서 공통 기능을 가진 클래스를 대체하는 기능
> - 특정 기능을 선언(설계)부분(=인터페이스)과 구현(동작)부분(=클래스)으로 분리한 후 구현부분을 다양한 방법으로 만들어 선택해서 사용할 수 있게하는 기능
> - 다형성을 구현하는 방법
>   1. **오버로딩**
>       - 메소드의 이름이 같아야 함
